...
- Citation density meets section targets (from quality_targets in paper-config.yml)
- Recency: meets configured recency_ratio (from quality_targets in paper-config.yml)
...
## Delivery checks
- Page count within target (from quality_targets.main_text_pages in paper-config.yml): from this skill folder, run `python3 scripts/compile_paper.py --project-dir <paper_dir> --report-page-counts` (requires a bibliography-start label; default `ReferencesStart`) and use "Main text pages (exclude ref-start page)".
- Citation count meets configured target (from quality_targets.total_citations in paper-config.yml).
