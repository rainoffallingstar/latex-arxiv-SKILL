...
## Goal
- Produce a review paper on <topic> using the configured template (Typst or LaTeX) (see paper-config.yml)
- Target 60-80 verified citations (70%+ from the last 3 years, or as set in quality_targets)
...
- **STOP**: Do not write prose into `main.typ` (or `main.tex`) until this gate is confirmed and the issues CSV exists.
- [ ] User confirmed scope + outline in chat
- Venue/template: <template_class> (from paper-config.yml)
