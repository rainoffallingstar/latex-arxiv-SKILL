...
每条引文在进入 `ref.bib` 之前必须经过**三级验证流水线**。未验证的引文默认在导出时被拒绝。
...
| 功能 | 说明 |
|---------|------|
| **自动级联** | DOI 失败后自动使用数据库元数据进行标题搜索重试 |
| **人工确认** | `--manual` 标志记录人工确认的通过结果，无需 API 调用 |
| **交互式提示** | 所有自动检查失败后，在命令行中直接选择下一步操作 |
| **批量验证** | `verify-all` 扫描整个数据库中的所有未验证引文 |
| **事实驱动检索** | 引文失败 → 提取事实 → 搜索替代论文 → 替换引文 |
| **强制门禁** | `export-bibtex` 默认拒绝未验证条目，除非使用 `--force` |
